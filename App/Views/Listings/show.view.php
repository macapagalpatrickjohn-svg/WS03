 <?php 
    ...
    $showOwnerActions = $currentUser !== null && Authorization::isOwner($listing->user_id);
?>
 <section class="container mx-auto p-4 mt-4">
      <div class="rounded-lg shadow-md bg-white p-3">
       <div class="flex justify-between items-center">
      <a class="block p-4 text-blue-700" href="<?= appRoot() ?>/listings">
        <i class="fa fa-arrow-alt-circle-left"></i>
        Back To Listings
      </a>
      <?php if ($showOwnerActions): ?>
      <div class="flex space-x-4 ml-4">
        <a href="<?= appRoot() ?>/listings/edit/<?= $listing->id ?>" class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded">Edit</a>
        <!-- Delete Form -->
    ...
    <section class="container mx-auto p-4">
      ...
      <div class="rounded-lg shadow-md bg-white p-4">
        ...
        <p><?= $listing->benefits ?? 'Not specified' ?></p>
      </div>
      <p class="my-5">
      ...
      <?php else: ?>
        <button
          type="button"
          disabled
          class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium text-gray-500 bg-gray-100 cursor-not-allowed"
        >
          Apply Now
        </button>
      <?php endif; ?>
